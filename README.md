...
pip install -e .  # in the top directory (`main` or `base`)
```

# Prepare Checkpoints
First, we need to generate a checkpoint file of ibert finetuned on the target downstream task.
While you can do this on the original Huggingface repository, 
you can run the following code to finetune the pre-trained model on GLUE tasks on `base` directory.
Make sure to install the model at `base` directory.

```
cd base/examples/text-classification
...
Make sure to install the model at `main` directory.
